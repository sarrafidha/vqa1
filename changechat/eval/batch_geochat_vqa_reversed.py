...
def eval_model(args):
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        args.model_path, args.model_base, "geochat_lora" + model_name
    )
    ...
    for i in tqdm(range(0, len(questions), args.batch_size)):
        ...
        for j in range(i, batch_end):
            image_file = list(reversed(questions[j]["image"]))
            qs = questions[j]["text"]
            ...
            if model.config.mm_use_im_start_end:
                ...
                qs = (
                    ...
                    + "\n"
                    + qs
                    + "Please focus only on obvious changes to buildings and roads, and ignore grass and vegetation. If only vegetation has changed or the objects that have changed are small, no change."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, default="experiments/geochat-cc-lora-0701"
    )
