    ...
    for i in tqdm(range(0,len(questions),args.batch_size)):
        ...
        for j in range(i,batch_end):
            ...
            if model.config.mm_use_im_start_end:
                ...
                qs = DEFAULT_IMAGE_TOKEN + '\n' + DEFAULT_IMAGE_TOKEN + '\n' + qs + 'Please draw the outlines of the changed buildings and roads, approximating them with polygons. Please remove unnecessary spaces in the polygon list.'
        ...
        image_tensor_batch = image_processor.preprocess(image_folder,crop_size ={'height': 252, 'width': 252},size = {'shortest_edge': 252}, return_tensors='pt')['pixel_values']

        with torch.inference_mode():
            output_ids = model.generate(final_input_tensors, images=image_tensor_batch.half().cuda(), do_sample=False, temperature=args.temperature, top_p=args.top_p, num_beams=1, max_new_tokens=1000,length_penalty=1, use_cache=True)
