...
def eval_model(args):
    ...
    for i in tqdm(range(0, len(questions), args.batch_size)):
        ...
        for j in range(i, batch_end):
            ...
            # qs = "Please describe the changes of these two images in detail."
            qs = ""
        ...
        if model.config.mm_projector_type == "single_stream_attention":
            chunks = torch.chunk(image_tensor_batch, chunks=args.batch_size, dim=0)
            image_tensor_batch = torch.cat([t.unsqueeze(0) for t in chunks], dim=0)

        with torch.inference_mode():
            # output_dict = model.forward(
            #     input_ids=final_input_tensors,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, default="experiments/changechat-pretrain-0801"
    )
