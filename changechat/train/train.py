...
from changechat.train.changechat_trainer import ChangeChatTrainer
from changechat import conversation as conversation_lib
...
def find_all_linear_names(model):
    ...
    # if "classifier_head" in lora_module_names:  # needed for 16-bit
    #     lora_module_names.remove("classifier_head")
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence["value"]:
                sentence["value"] = (
                    sentence["value"]
                    .replace(DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN + "\n")
                    .strip()
                )
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            if images[0].ndim == 4:
                batch["images"] = torch.cat(images, dim=0)  # (2xb, c, h, w)
            else:
                batch["images"] = torch.stack(images)  # (b,2,c,h,w) for change chat

        # if "change_labels" in instances[0]:
...
def train():
    ...
    if model_args.vision_tower is not None:
        model = ChangeChatLlamaForCausalLM.from_pretrained(
    ...
    model.config.classifier_loss_weight = model_args.classifier_loss_weight
    ...
    if training_args.lora_enable:
        print(
            f"###########lora_r: {training_args.lora_r}, lora_alpha: {training_args.lora_alpha}###########"
        )
    ...
    # if model_args.classifier_loss_weight > 0:
    #     model.classifier_head.requires_grad_(True)
