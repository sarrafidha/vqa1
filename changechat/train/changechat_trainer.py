...
class ChangeChatTrainer(Trainer):

    def _get_train_sampler(self, train_dataset=None, **kwargs) -> Optional[torch.utils.data.Sampler]:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            super(ChangeChatTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            pass
        else:
            super(ChangeChatTrainer, self)._save(output_dir, state_dict)


class ChangeClassifierTrainer(Trainer):
    def _save_checkpoint(self, model, trial, metrics=None):
        from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
