...
class Chat:
    def __init__(
        self, model, image_processor, tokenizer, device="cuda:0", stopping_criteria=None
    ):
        self.device = device
        self.model = model
    ...
    def answer_prepare(
        ...
        stopping_criteria = KeywordsStoppingCriteria(
            keywords, self.tokenizer, text_input_ids
        )
    ...
    def model_generate(self, *args, **kwargs):
        ...
        with torch.inference_mode():
            ...
            outputs = self.tokenizer.decode(
                output[0, kwargs["kwargs"]["input_ids"].shape[1] :]
            ).strip()
            # print(outputs)
        return output
