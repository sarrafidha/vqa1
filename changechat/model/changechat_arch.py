...
class ChangeChatMetaModel:

    def __init__(self, config):
        super(ChangeChatMetaModel, self).__init__(config)

        # OFFLINE MODE FOR HuggingFace
        self.config.mm_vision_tower = "hf-models/clip-vit-large-patch14-336"
...
class ChangeChatMetaForCausalLM(ABC):
    ...
    def encode_images_without_projector(self, images):
        image_features = self.get_model().get_vision_tower()(images)
        return image_features

    def encode_images_only_projector(self, images):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if mm_projector_type == "single_stream_attention":
            assert images.ndim == 5
            concat_images = torch.cat([image for image in images], dim=0)
            image_features = self.encode_images_without_projector(concat_images)
            split_sizes = [image.shape[0] for image in images]
            image_features = torch.split(
                image_features, split_sizes, dim=0
            )  # b tuples of [2, N, L]
            image_features = torch.stack(image_features, dim=0)
            change_pred, image_features = self.encode_images_only_projector(
                image_features
            )
